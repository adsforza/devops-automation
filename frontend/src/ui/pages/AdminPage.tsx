...
export function AdminPage() {
	...
	const deleteMut = useMutation({ mutationFn: (id: string) => deleteUser(id), onSuccess: () => { invalidateUsers(); toast({ title: 'Usuario eliminado', status: 'success' }); } });
	...
	const rolesDeleteMut = useMutation({ mutationFn: (id: string) => rolesDelete(id), onSuccess: () => { qc.invalidateQueries({ queryKey: ['roles-all'] }); toast({ title: 'Rol eliminado', status: 'success' }); } });
	...
	const connDeleteMut = useMutation({ mutationFn: (id: string) => deleteConnection(id), onSuccess: () => { qc.invalidateQueries({ queryKey: ['connections-all'] }); toast({ title: 'Conexión eliminada', status: 'success' }); } });
				...
				<TabPanels>
					<TabPanel>
						<HStack mb={4} spacing={4} align="flex-end">
							<FormControl maxW="sm"><FormLabel>Email</FormLabel><Input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="user@example.com" /></FormControl>
							<FormControl maxW="sm"><FormLabel>Nombre</FormLabel><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nombre Apellido" /></FormControl>
					...
					<TabPanel>
						<HStack mb={4} spacing={4} align="flex-end">
							<FormControl maxW="sm"><FormLabel>Nombre</FormLabel><Input value={newRoleName} onChange={(e) => setNewRoleName(e.target.value)} /></FormControl>
							...
							<Button colorScheme="blue" onClick={() => rolesCreateMut.mutate({ name: newRoleName, description: newRoleDesc })} isLoading={rolesCreateMut.isPending}>Crear rol</Button>
						</HStack>
						<Table size="sm"><Thead><Tr><Th>Nombre</Th><Th>Descripción</Th><Th>Acciones</Th></Tr></Thead><Tbody>{(rolesAll || []).map((r: any) => (<Tr key={r.id}><Td><Input size="sm" defaultValue={r.name} onBlur={(e) => rolesUpdateMut.mutate({ id: r.id, payload: { name: e.target.value } })} /></Td><Td><Input size="sm" defaultValue={r.description || ''} onBlur={(e) => rolesUpdateMut.mutate({ id: r.id, payload: { description: e.target.value } })} /></Td><Td><IconButton aria-label="Eliminar" size="sm" colorScheme="red" icon={<DeleteIcon />} onClick={() => rolesDeleteMut.mutate(r.id)} /></Td></Tr>))}{(!rolesAll || rolesAll.length === 0) && <Tr><Td colSpan={3}>Sin roles</Td></Tr>}</Tbody></Table>
					</TabPanel>
					<TabPanel>
						<HStack mb={4} spacing={4} align="flex-end">
